...
export function inferAccessProfileDetails({
  ...
  if (/onclick\s*=|javascript:|data-href|data-url|data-link/.test(lowered)) {
    addProfile(
      ACCESS_PROFILES.STATIC_HTML_EVENT_URL,
      makeProfileEvidence("list HTML contains event/data-url navigation hints", "html_hint", "onclick/javascript/data-url"),
    );
  }
  ...
  if (/pagination|pageNo|pageIndex|paging|paginate|next|prev/i.test(body)) {
    addProfile(
      ...
      makeProfileEvidence("list HTML contains pagination markers", "html_hint", "pagination/pageNo/pageIndex"),
    );
  }
  if (
