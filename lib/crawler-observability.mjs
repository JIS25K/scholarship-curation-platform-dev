...
export function verifyDetailTitleIdentity(listTitle, detailTitleCandidates = [], options = {}) {
  ...
  if (listTitleQuality !== "clean") {
    return {
      verified: false,
      status: "list_title_not_clean",
      comparisonMode: "list_title_quality_failed",
      evidence: `List title quality is ${listTitleQuality || "unknown"}.`,
    };
  }
...
export function decidePrimaryFailureCode({
  ...
  if (fallbackScanUsed) return FAILURE_CODES.CONFIG_OR_SELECTOR_MISMATCH;
...
export function makeSourceDecision({
  ...
  detailFetchSuccessCount = 0,
  detailUrlVerifiedCount = 0,
  detailContentCharCount = 0,
  cleanDetailSampleCount = 0,
  contaminatedCandidateLeakCount = 0,
  minDetailContentCharCount = 80,
}) {
  ...
  if (
    ...
    Number(cleanDetailSampleCount) >= 1 &&
    Number(contaminatedCandidateLeakCount) === 0 &&
    Number(detailContentCharCount) >= Number(minDetailContentCharCount)
  ) {
    return "supported";
  }
